...
int setMode(CDC& dc, CRect& rectWindow, CRectD& rectWorld) //режим отображение
{
	...
	int buffer = dc.SetMapMode(MM_TEXT);//Логические единицы измерения отображаются произвольными единицами измерения с произвольно масштабируемыми осями
...
void CPlot2D::plot(CDC& dc, bool drawOuterRect, bool drawInnerGrid)//рисуем графики
{
	...
	if (drawInnerGrid)//русуем график
	{
		...
		CPen pen_dop(_penAxis._style, _penAxis._width + 1, _penAxis._color);//передаем параметры пера
		CPen* pOldPen_dop = dc.SelectObject(&pen_dop); // Выбираем перо в контекст памяти

		dc.MoveTo(_rectWindow.left, _rectWindow.top);
		dc.LineTo(_rectWindow.left, _rectWindow.bottom + 20);
		dc.MoveTo(_rectWindow.left, _rectWindow.top);
		dc.LineTo(_rectWindow.right + 20, _rectWindow.top);
